...
<div class="content-body">
	<!-- row -->
	<div class="container-fluid"><strong></strong>

		<div class="form-head d-flex align-items-center mb-sm-4 mb-3">
			<div class="mr-auto">
				<h2 class="text-black font-w600">Dashboard</h2>
				<p class="mb-0">Online COVID TEST and VACCINATION Booking System </p>
			</div>
		</div>
		<?php
		include 'dbcon.php';
		$sql = "SELECT * FROM hp_hospital";
		$query = mysqli_query($con, $sql);
		$num = mysqli_num_rows($query);

		$sql_p = "SELECT * FROM hp_patient";
		$query_p = mysqli_query($con, $sql_p);
		$num_p = mysqli_num_rows($query_p);

		$sql_t = "SELECT * FROM hp_patient where p_treatment = 'covid test' ";
		$query_t = mysqli_query($con, $sql_t);
		$num_t = mysqli_num_rows($query_t);

		$sql_v = "SELECT * FROM hp_patient where p_treatment = 'covid vaccination' ";
		$query_v = mysqli_query($con, $sql_v);
		$num_v = mysqli_num_rows($query_v);
			...
			<div class="col-xl-3 col-sm-6">
				<div class="card">
					<div class="card-body">
						<div class="media align-items-center">
							<div class="media-body mr-3">
								<h2 class="fs-34 text-black font-w600"><?php echo $num ?></h2>
					...
					<div class="progress  rounded-0" style="height:4px;">
						<div class="progress-bar rounded-0 bg-secondary progress-animated" style="width: <?php echo $num ?>%; height:4px;" role="progressbar">
			...
			<div class="col-xl-3  col-sm-6">
				<div class="card">
					<div class="card-body">
						<div class="media align-items-center">
							<div class="media-body mr-3">
								<h2 class="fs-34 text-black font-w600"><?php echo $num_p ?></h2>
					...
					<div class="progress  rounded-0" style="height:4px;">
						<div class="progress-bar rounded-0 bg-secondary progress-animated" style="width: <?php echo $num_p ?>%; height:4px;" role="progressbar">
							<span class="sr-only"><?php echo $num_p ?>% Complete</span>
						</div>
					</div>
				</div>
			</div>
			<div class="col-xl-3  col-sm-6">
				<div class="card">
					<div class="card-body">
						<div class="media align-items-center">
							<div class="media-body mr-3">
								<h2 class="fs-34 text-black font-w600"><?php echo $num_t ?></h2>
					...
					<div class="progress  rounded-0" style="height:4px;">
						<div class="progress-bar rounded-0 bg-secondary progress-animated" style="width: <?php echo $num_t ?>%; height:4px;" role="progressbar">
							<span class="sr-only"><?php echo $num_t ?>% Complete</span>
						</div>
					</div>
				</div>
			</div>
										...
										<div class="d-flex flex-wrap align-items-center px-4 bg-light">
											<div class="mr-auto d-flex align-items-center py-3">
												...
												<div>
													...
													<span class="fs-26 text-primary font-w600"><?php echo $num_p ?></span>
												</div>
											</div>
											<ul class="users pl-3 py-2">
												<?php
												$query_pics = mysqli_query($con, "select * from hp_patient limit 4");

												while ($data = mysqli_fetch_assoc($query_pics)) {  ?>
													<li><img src="<?php echo $data['p_image'] ?>" alt=""></li>
												<?php } ?>
											</ul>
										</div>
											...
											<div class="col-xl-6 col-xxl-12 col-md-6">
												<div class="d-flex mb-4 align-items-center">
													<span class="mr-auto pr-3 font-w500 fs-30 text-black">
														...
														<?php
														$new = mysqli_query($con, "select * from hp_patient where p_status = 'Pending' ");
														$ress = mysqli_num_rows($new);
                                                        // $jsres = "<script>document.write($ress)</script>";

														echo '<span id="pending_id">'.$ress. '</span>%';
														?>
													</span>
												...
												<div class="d-flex  mb-4 align-items-center">
													<span class="mr-auto pr-3 font-w500 fs-30 text-black">
														...
														<?php
														$new = mysqli_query($con, "select * from hp_patient where p_status = 'Vaccinated' OR p_status = 'Negative'");
														$ress = mysqli_num_rows($new);
														echo '<span id="recoverd_id">' . $ress . '</span>%';
														?>
													</span>
													<span>Recovered</span>
												</div>
												<div class="d-flex align-items-center">
													<span class="mr-auto pr-3 font-w500 fs-30 text-black">
														...
														<?php
														$new = mysqli_query($con, "select * from hp_patient where p_status = 'Treatment' ");
														$ress = mysqli_num_rows($new);
														echo '<span id="treatment_id">' . $ress . '</span>%';
														?>
													</span>
				...
				<div class="col-xl-12">
					<div class="card rated-doctors">
						<div class="card-header border-0 pb-0">
							<h3 class="fs-20 text-black mb-0 mr-auto">Top Hospitals</h3>
						</div>
						<div class="card-body">
							<?php
							$hospitals = mysqli_query($con, "select * from hp_hospital where status = 'true'  limit 5");

							while ($data = mysqli_fetch_assoc($hospitals)) {  ?>
								...
								<div class="d-sm-flex pb-sm-4 pb-3 border-bottom mb-sm-4 mb-3 align-items-center">
									<div class="d-flex align-items-center mr-auto pr-2">
										<span class="num mr-sm-4 mr-3">#<?php echo $data['id'] ?></span>
										<img src="<?php echo $data['logo'] ?>" class="img-1 mr-sm-4 mr-3" alt="">
										<div>
											<h4 class="mb-sm-2 mb-1 text-black"><?php echo $data['name'] ?></h4>
											<a href="<?php echo $data['web'] ?>" target="blank" class="fs-14 text-primary font-w600"><?php echo $data['web'] ?></a>
										</div>
									</div>
								</div>
							<?php } ?>
